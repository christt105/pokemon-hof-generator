...
signal trainer_chosen(file_name: String, texture: Texture2D)

const TrainerCatalog = preload("res://scripts/trainer_catalog.gd")
...
var _catalog := TrainerCatalog.new()
var _all_families: Array[Dictionary] = []
var _visible_families: Array[Dictionary] = []
var _selected_family: Dictionary = {}
var _selected_variant: Dictionary = {}
var _variant_button_group := ButtonGroup.new()
var _fill_token := 0  # bumped on every new filter so a stale batched fill bails out


func _ready() -> void:
	_all_families = _catalog.load_families()
...
func _populate_category_options() -> void:
	...
	for category: String in TrainerCatalog.CATEGORY_ORDER:
		var count := 0
		for family: Dictionary in _all_families:
			if family["category"] == category:
				count += 1
		category_option.add_item("%s (%d)" % [TrainerCatalog.CATEGORY_LABELS[category], count])
...
func _refresh_family_list() -> void:
	...
	_visible_families = _all_families.filter(func(family: Dictionary) -> bool:
...
func _fill_family_list_batched(token: int) -> void:
	...
	while index < _visible_families.size():
		...
		for i in range(index, batch_end):
			var family: Dictionary = _visible_families[i]
			var cover: Dictionary = family["variants"][0]
			var texture: Texture2D = load(cover["path"])
			...
			if family["variants"].size() > 1:
				family_list.set_item_tooltip(item_index, "%d variantes" % family["variants"].size())
		index = batch_end
...
func _populate_variant_row() -> void:
	...
	for variant: Dictionary in _selected_family["variants"]:
		var button := TextureButton.new()
		button.texture_normal = load(variant["path"])
...
func _select_variant(variant: Dictionary) -> void:
	_selected_variant = variant
	var texture: Texture2D = load(variant["path"])
...
func _on_confirm_pressed() -> void:
	trainer_chosen.emit(_selected_variant["file"], preview_texture.texture)
